...
def tif2zarr(tiff_dir, tiff_dir_2, zarr_path, chunk_size = (64, 256, 256)):
    
    tiff_dir = Path(tiff_dir)
    zarr_path = Path(zarr_path)

    if tiff_dir_2 is not None:
        tiff_dir_2 = Path(tiff_dir_2)
        print(f"Loading channel 1 from {tiff_dir} and channel 2 from {tiff_dir_2}") 
    ...
    if tiff_dir_2 is not None:
        tif_files_2 = list(tiff_dir_2.glob("*.tif"))
        if not tif_files_2:
            raise FileNotFoundError(f"No .tif files found in {tiff_dir_2}")
        if len(tif_files) != len(tif_files_2):
            raise ValueError(f"Number of TIFF files in {tiff_dir} and {tiff_dir_2} do not match.")
    ...
    if tiff_dir_2 is  None:
        
        im = imread_dask(str(tiff_dir / "*.tif"))
        print(f"Input array shape: {im.shape}, dtype: {im.dtype}")
        print(f"Input chunk sizes: {im.chunksize}")
    else:
        im_ch1 = imread_dask(str(tiff_dir / "*.tif"))
        im_ch2 = imread_dask(str(tiff_dir_2 / "*.tif"))
        # Stack along a new channel axis
        im = da.stack([im_ch1, im_ch2], axis=0)  # New shape: (C,Z, Y, X)
...
def main():       
    ...
    parser.add_argument("-i2", '--input_tiff_path_ch2', help="Directory of raw tiff slices for the second channel", required=False, default=None)
    ...
    tiff_dir_2 = args.input_tiff_path_ch2
    chunk_size = args.chunk_size
    with dask.config.set(scheduler='threads', num_workers=args.workers):
        tif2zarr(tiff_dir,tiff_dir_2, zarr_path, chunk_size)
